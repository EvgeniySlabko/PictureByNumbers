...
import { FacetCreator, FacetResult } from "./picture-by-number-generation/FacetCreator";
...
export class GUIProcessManager {
    static async process(settings: Settings, cancellationToken: CancellationToken, canvas: HTMLCanvasElement) {
            const ctx = canvas.getContext("2d");
            let imgData = ctx.getImageData(0, 0, canvas.width, canvas.height);
            if (settings.resizeImageIfTooLarge && (canvas.width > settings.resizeImageWidth || canvas.height > settings.resizeImageHeight)) {
            ...
            // k-means clustering
            const kmeansImgData = await GUIProcessManager.processKmeansClustering(imgData, null, ctx, settings, cancellationToken, canvas);
            ...
            if (settings.narrowPixelStripCleanupRuns === 0) {
                ...
                // facet reduction
                await GUIProcessManager.processFacetReduction(facetResult, null, settings, colormapResult, cancellationToken, canvas);
            }
            else {
            ...
            await GUIProcessManager.processFacetBorderTracing(null, facetResult, cancellationToken, canvas);
            // facet border segmentation
            const cBorderSegment = await GUIProcessManager.processFacetBorderSegmentation(facetResult, null, settings, cancellationToken, canvas);
            // facet label placement
            await GUIProcessManager.processFacetLabelPlacement(facetResult, cBorderSegment, null, cancellationToken, canvas);
    ...
    static async processKmeansClustering(imgData, tabsOutput, ctx, settings, cancellationToken, canvas: HTMLCanvasElement) {
            ...
            const kmeansImgData = ctxKmeans.getImageData(0, 0, canvas.width, canvas.height);
    ...
    static async processFacetBuilding(colormapResult, cancellationToken) {
    ...
    static async processFacetReduction(facetResult, tabsOutput, settings, colormapResult, cancellationToken, canvas: HTMLCanvasElement) {
            ...
            const reductionImgData = ctxReduction.getImageData(0, 0, canvas.width, canvas.height);
    ...
    static async processFacetBorderTracing(tabsOutput, facetResult, cancellationToken, canvas: HTMLCanvasElement) {
    ...
    static async processFacetBorderSegmentation(facetResult, tabsOutput, settings, cancellationToken, canvas: HTMLCanvasElement) {
    ...
    static async processFacetLabelPlacement(facetResult, cBorderSegment, tabsOutput, cancellationToken, canvas: HTMLCanvasElement) {
     ...
     */
    static async createSVG(svg: SVGSVGElement, facetResult, colorsByIndex, sizeMultiplier, fill, stroke, addColorLabels, fontSize = 50, fontColor = "black", onUpdate = null) {
