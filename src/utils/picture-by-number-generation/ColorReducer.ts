import { hslToRgb, lab2rgb, rgb2lab, rgbToHsl } from "./common/color-helpers";
import { delay } from "./common/delay";
import { KMeans } from "./KMeans";
import { Random } from "./common/Random";
import { ClusteringColorSpace, Settings } from "./common/Settings";
import { Uint8Array2D } from "./common/arrays/Uint8Array2D";
import { Vector } from "./common/Vector";
import { ColorMapResult } from "./common/ColorMapResult";
...
export class ColorReducer {
  ...
  static createColorMap(kmeansImgData: ImageData): ColorMapResult {
    ...
    const colorsByIndex: [number, number, number][] = [];
    let idx = 0;
    ...
    return {
      imgColorIndices: imgColorIndices,
      colorsByIndex: colorsByIndex,
      width: kmeansImgData.width,
      height: kmeansImgData.height,
    };
  }
  /**
   ...
   */
  static async applyKMeansClustering(
    imgData: ImageData,
    outputImgData: ImageData,
    settings: Settings,
    onUpdate: (kMeans: KMeans) => void,
  ) {
    ...
    const pointsByColor: { [key: string]: number[] } = {};
  ...
  static updateKmeansOutputImageData(
    kmeans: KMeans,
    settings: Settings,
    pointsByColor: { [key: string]: number[] },
    imgData: ImageData,
    outputImgData: ImageData,
    ...
    for (let c = 0; c < kmeans.centroids.length; c++) {
      ...
      for (const v of kmeans.pointsPerCategory[c]) {
        ...
        // remove decimals
        rgb = rgb.map((v) => Math.floor(v)) as [number, number, number];
   ...
   */
  static buildColorDistanceMatrix(colorsByIndex: [number, number, number][]) {
    const colorDistances = new Array<Array<number>>(colorsByIndex.length);
  ...
  static processNarrowPixelStripCleanup(colormapResult: ColorMapResult) {
