...
export class FacetBorderTracer {
   ...
   */
  static async buildFacetBorderPaths(
    facetResult: FacetResult,
    onUpdate: (progess: number) => void,
  ) {
    let count = 0;
    const borderMask = new BooleanArray2D(
      facetResult.width,
      facetResult.height,
    );
    ...
    for (let fidx = 0; fidx < facetProcessingOrder.length; fidx++) {
      ...
      if (f != null) {
        ...
        const xWall = new BooleanArray2D(
          facetResult.width + 1,
          facetResult.height + 1,
        );
        const yWall = new BooleanArray2D(
          facetResult.width + 1,
          facetResult.height + 1,
        );
