...
export class FacetBorderTracer {
     ...
     */
    static async buildFacetBorderPaths(facetResult: any, onUpdate: Function | null = null) {
        let count = 0;
        const borderMask = new BooleanArray2D(facetResult.width, facetResult.height);
        // sort by biggest facets first
        const facetProcessingOrder = facetResult.facets.filter((f: any) => f != null).slice(0).sort((a: any, b: any) => b.pointCount > a.pointCount ? 1 : (b.pointCount < a.pointCount ? -1 : 0)).map((f: any) => f.id);
     ...
     */
    static getPath(pt: any, facetResult: any, f: any, borderMask: any, xWall: any, yWall: any) {
        const debug = false;
        let finished = false;
        const count = 0;
        const path: any = [];
     ...
     */
    static addPointToPath(path: any, pt: PathPoint, xWall: any, f: any, yWall: any) {
        path.push(pt);
