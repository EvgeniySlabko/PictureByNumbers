...
import { pointToPolygonDist, polylabel } from "./polyLabel";
...
export class FacetLabelPlacer {
   ...
   */
  static async buildFacetLabelBounds(facetResult: FacetResult, onUpdate: (progress: number) => void) {
    let count = 0;
  ...
  static doesNeighbourFallInsideInCurrentFacet(
    ...
    if (fallsInside) {
      ...
      for (let i = 0; i < neighbourPath.length && fallsInside; i++) {
        const distance = pointToPolygonDist(
          neighbourPath[i].x,
          neighbourPath[i].y,
          onlyOuterRing,
        );
