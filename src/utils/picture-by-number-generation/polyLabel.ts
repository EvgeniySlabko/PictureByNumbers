...
export function polylabel(polygon: Point[][], precision = 1.0) {
...
export function pointToPolygonDist(x: number, y: number, polygon: Point[][]) {
  let inside = false;
...
function getCentroidCell(polygon: Point[][]) {
  let area = 0;
  let x = 0;
  let y = 0;
