...
export class FacetBorderSegmenter {
  ...
  static async buildFacetBorderSegments(
    ...
    await FacetBorderSegmenter.matchSegmentsWithNeighbours(
      facetResult,
      segmentsPerFacet,
      onUpdate,
    );
   ...
   */
  static prepareSegmentsPerFacet(facetResult: FacetResult): PathSegment[][] {
    const segmentsPerFacet: PathSegment[][] = new Array<PathSegment[]>(facetResult.facets.length);

    for (const facet of facetResult.facets) {
      if (facet != null) {
        const segments: PathSegment[] = [];
        if (facet.borderPath.length > 1) {
          let currentPoints: PathPoint[] = [];
          currentPoints.push(facet.borderPath[0]);

          for (let i = 1; i < facet.borderPath.length; i++) {
            const prevBorderPoint = facet.borderPath[i - 1];
            const curBorderPoint = facet.borderPath[i];
            ...
            if (oldNeighbour !== curNeighbour) {
              ...
              if (
                FacetBorderSegmenter.isTransitionDueToDiagonals(
                  prevBorderPoint,
                  curBorderPoint,
                  facetResult,
                  oldNeighbour,
          ...
          if (currentPoints.length > 1) {
            const oldNeighbour =
              facet.borderPath[facet.borderPath.length - 1].getNeighbour(facetResult);
        ...
        segmentsPerFacet[facet.id] = segments;
      }
    }
  ...
  private static isTransitionDueToDiagonals(
    prevBorderPoint: PathPoint,
    curBorderPoint: PathPoint,
    facetResult: FacetResult,
    oldNeighbour: number,
  ): boolean {
    if (
  ...
  static reduceSegmentHaarWavelet(
    ...
    for (let i = 1; i < newpath.length - 2; i += 2) {
      if (
        ...
        reducedPath.push(
          new PathPoint(new Point(cx, cy), OrientationEnum.Left),
        );
      } else {
              ...
              if (neighbourFacet != null) {
                ...
                for (let ns = 0; ns < neighbourSegments.length; ns++) {
                  ...
                  if (
                    ...
                    const reverse = FacetBorderSegmenter.shouldReverse(
                      segment,
                      neighbourSegment,
                    );
                    neighbourFacet.borderSegments[ns] =
                      new FacetBoundarySegment(segment, f.id, reverse);
  ...
  private static areSegmentsMatching(
    ...
    const nSegEndPoint =
      neighbourSegment.points[neighbourSegment.points.length - 1];
  ...
  private static shouldReverse(segment: any, neighbourSegment: any): boolean {
    ...
    return (
      segStartPoint.distanceTo(nSegEndPoint) <= 4 &&
      segEndPoint.distanceTo(nSegStartPoint) <= 4
    );
  }
}
