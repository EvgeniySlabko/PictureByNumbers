import { BooleanArray2D } from "./common/arrays/BooleanArray2D";
import { delay } from "./common/delay";
import { FacetCreator, FacetResult } from "./FacetCreator";
...
export class FacetReducer {
  ...
  static async reduceFacets(
    ...
    const visitedCache = new BooleanArray2D(
      facetResult.width,
      facetResult.height,
    );
  ...
  static deleteFacet(
    ...
    visitedArrayCache: BooleanArray2D,
  ) {
  ...
  static rebuildChangedNeighbourFacets(
    ...
    const changedNeighboursSet: {[key: number]: boolean} = {};
    if (facetToRemove.neighbourFacetsIsDirty) {
      FacetCreator.buildFacetNeighbour(facetToRemove, facetResult);
    }
    for (const neighbourIdx of facetToRemove.neighbourFacets) {
