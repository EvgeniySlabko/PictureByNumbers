...
define("lib/clustering", ["require", "exports"], function (require, exports) {
  ...
  class Vector {
    ...
    distanceTo(p) {
      ...
      for (let i = 0; i < this.values.length; i++) {
        sumSquares +=
          (p.values[i] - this.values[i]) * (p.values[i] - this.values[i]);
      }
  ...
  class KMeans {
    ...
    initCentroids() {
      for (let i = 0; i < this.k; i++) {
        this.centroids.push(
          this.points[Math.floor(this.points.length * this.random.next())],
        );
        this.pointsPerCategory.push([]);
      }
    }
...
define("settings", ["require", "exports"], function (require, exports) {
  ...
  (function (ClusteringColorSpace) {
    ...
    (ClusteringColorSpace =
      exports.ClusteringColorSpace || (exports.ClusteringColorSpace = {})),
  );
  class Settings {
    constructor() {
...
define("colorreductionmanagement", [
  "require",
  "exports",
  "common",
  "lib/clustering",
  ...
  class ColorReducer {
    ...
    static createColorMap(kmeansImgData) {
      const imgColorIndices = new typedarrays_1.Uint8Array2D(
        kmeansImgData.width,
    ...
    static applyKMeansClustering(
      ...
      return __awaiter(this, void 0, void 0, function* () {
        ...
        for (const color of Object.keys(pointsByColor)) {
          ...
          let data;
          if (
            settings.kMeansClusteringColorSpace ===
            settings_1.ClusteringColorSpace.RGB
          ) {
            data = rgb;
          } else if (
            settings.kMeansClusteringColorSpace ===
            settings_1.ClusteringColorSpace.HSL
          ) {
            data = colorconversion_1.rgbToHsl(rgb[0], rgb[1], rgb[2]);
          } else if (
            settings.kMeansClusteringColorSpace ===
            settings_1.ClusteringColorSpace.LAB
          ) {
        ...
        const kmeans = new clustering_1.KMeans(
          vectors,
        ...
        while (
          ...
          settings.kMeansMinDeltaDifference
        ) {
          kmeans.step();
     ...
     */
    static updateKmeansOutputImageData(
      kmeans,
      settings,
      pointsByColor,
      imgData,
      outputImgData,
      restrictToSpecifiedColors,
    ) {
            ...
            if (settings.kMeansColorRestrictions.length > 0) {
              ...
              for (const color of settings.kMeansColorRestrictions) {
                ...
                if (typeof color === "string") {
                  restrictionLab = colorconversion_1.rgb2lab(
                    settings.colorAliases[color],
                  );
                } else {
...
define("facetmanagement", ["require", "exports", "structs/point"], function (
  ...
  (function (OrientationEnum) {
    ...
    (OrientationEnum =
      exports.OrientationEnum || (exports.OrientationEnum = {})),
  );
  /**
...
define("facetBorderSegmenter", [
  ...
  class FacetBorderSegmenter {
     ...
     */
    static buildFacetBorderSegments(
      facetResult,
            ...
            for (let i = 1; i < f.borderPath.length; i++) {
              ...
              if (oldNeighbour !== curNeighbour) {
                ...
                if (oldNeighbour !== -1) {
                  ...
                  if (
                    prevBorderPoint.x === curBorderPoint.x &&
     ...
     */
    static reduceSegmentComplexity(
      facetResult,
      segmentsPerFacet,
      nrOfTimesToHalvePoints,
    ) {
    ...
    static reduceSegmentHaarWavelet(
      ...
      for (let i = 1; i < newpath.length - 2; i += 2) {
        if (
          !skipOutsideBorders ||
          (skipOutsideBorders &&
            !FacetBorderSegmenter.isOutsideBorderPoint(
              newpath[i],
              width,
              height,
            ))
          ...
          reducedPath.push(
            new facetmanagement_1.PathPoint(
              new point_2.Point(cx, cy),
              facetmanagement_1.OrientationEnum.Left,
            ),
          );
        } else {
     ...
     */
    static matchSegmentsWithNeighbours(
      facetResult,
      segmentsPerFacet,
      onUpdate = null,
    ) {
          ...
          if (f != null) {
            ...
            for (let s = 0; s < segmentsPerFacet[f.id].length; s++) {
              ...
              if (segment != null && f.borderSegments[s] == null) {
                f.borderSegments[s] = new FacetBoundarySegment(
                  segment,
                  segment.neighbour,
                  false,
                );
                ...
                if (segment.neighbour !== -1) {
                  ...
                  if (neighbourFacet != null) {
                    const neighbourSegments =
                      segmentsPerFacet[segment.neighbour];
      ...
      return __awaiter(this, void 0, void 0, function* () {
        ...
        for (let fidx = 0; fidx < facetProcessingOrder.length; fidx++) {
          ...
          if (f != null) {
            ...
            const xWall = new typedarrays_2.BooleanArray2D(
              facetResult.width + 1,
              facetResult.height + 1,
            );
            const yWall = new typedarrays_2.BooleanArray2D(
              facetResult.width + 1,
              facetResult.height + 1,
            );
            ...
            const pt = new facetmanagement_2.PathPoint(
              f.borderPoints[borderStartIndex],
              facetmanagement_2.OrientationEnum.Left,
            );
            // L T R B
            if (
              pt.x - 1 < 0 ||
            ...
            const path = FacetBorderTracer.getPath(
              pt,
              facetResult,
              f,
              borderMask,
    ...
    static getPath(pt, facetResult, f, borderMask, xWall, yWall) {
      ...
      while (!finished) {
        ...
        if (pt.orientation === facetmanagement_2.OrientationEnum.Left) {
          ...
          if (
            ((pt.y - 1 >= 0 &&
              facetResult.facetMap.get(pt.x, pt.y - 1) !== f.id) || // top exists and is a neighbour facet
            ...
            const nextpt = new facetmanagement_2.PathPoint(
              new point_3.Point(pt.x, pt.y),
              facetmanagement_2.OrientationEnum.Top,
            );
          ...
          if (
            ((pt.y + 1 < facetResult.height &&
              facetResult.facetMap.get(pt.x, pt.y + 1) !== f.id) || // bottom exists and is a neighbour facet
            ...
            const nextpt = new facetmanagement_2.PathPoint(
              new point_3.Point(pt.x, pt.y),
              facetmanagement_2.OrientationEnum.Bottom,
            );
            possibleNextPoints.push(nextpt);
          }
          // check upwards
          ...
          if (
            ...
            (pt.x - 1 < 0 ||
              facetResult.facetMap.get(pt.x - 1, pt.y - 1) !== f.id) && // and
          ...
          if (
            ...
            const nextpt = new facetmanagement_2.PathPoint(
              new point_3.Point(pt.x - 1, pt.y - 1),
              facetmanagement_2.OrientationEnum.Bottom,
            );
            possibleNextPoints.push(nextpt);
          }
          ...
          if (
            ...
            const nextpt = new facetmanagement_2.PathPoint(
              new point_3.Point(pt.x, pt.y),
              facetmanagement_2.OrientationEnum.Right,
            );
            possibleNextPoints.push(nextpt);
          }
          // check leftwards
  ...
  class FacetReducer {
    ...
    static deleteFacet(
      ...
      if (facetToRemove.neighbourFacetsIsDirty) {
        facetCreator_1.FacetCreator.buildFacetNeighbour(
          facetToRemove,
          facetResult,
        );
      }
      if (facetToRemove.neighbourFacets.length > 0) {
        ...
        // a facet is rebuilt
        for (
          let j = facetToRemove.bbox.minY;
          j <= facetToRemove.bbox.maxY;
          j++
        ) {
          for (
            let i = facetToRemove.bbox.minX;
            i <= facetToRemove.bbox.maxX;
            i++
          ) {
            if (facetResult.facetMap.get(i, j) === facetToRemove.id) {
              ...
              if (closestNeighbour !== -1) {
                ...
                imgColorIndices.set(
                  i,
                  j,
                  facetResult.facets[closestNeighbour].color,
                );
              } else {
      ...
      FacetReducer.rebuildForFacetChange(
        visitedArrayCache,
        facetToRemove,
        imgColorIndices,
        facetResult,
    ...
    static rebuildForFacetChange(
      visitedArrayCache,
      facet,
      imgColorIndices,
      facetResult,
    ) {
      FacetReducer.rebuildChangedNeighbourFacets(
        visitedArrayCache,
        facet,
        imgColorIndices,
        facetResult,
          ...
          if (facetResult.facetMap.get(x, y) === facet.id) {
            ...
            if (
              ...
              imgColorIndices.set(
                x,
                y,
                facetResult.facets[facetResult.facetMap.get(x - 1, y)].color,
              );
            } else if (
              y - 1 >= 0 &&
              ...
              imgColorIndices.set(
                x,
                y,
                facetResult.facets[facetResult.facetMap.get(x, y - 1)].color,
              );
            } else if (
              x + 1 < facetResult.width &&
              facetResult.facetMap.get(x + 1, y) !== facet.id &&
              facetResult.facets[facetResult.facetMap.get(x + 1, y)] !== null
            ) {
              imgColorIndices.set(
     ...
     */
    static getClosestNeighbourForPixel(
      facetToRemove,
      facetResult,
      x,
      y,
        ...
        if (neighbour != null) {
          for (const bpt of neighbour.borderPoints) {
            ...
            if (distance < minDistance) {
              ...
              const colorDistance =
                colorDistances[facetToRemove.color][neighbour.color];
     ...
     */
    static rebuildChangedNeighbourFacets(
      visitedArrayCache,
      facetToRemove,
      imgColorIndices,
      facetResult,
      ...
      for (const neighbourIdx of facetToRemove.neighbourFacets) {
        ...
        if (neighbour != null) {
          ...
          const newFacet = facetCreator_1.FacetCreator.buildFacet(
            neighbourIdx,
            neighbour.color,
  ...
  class FacetCreator {
    ...
    static getFacets(width, height, imgColorIndices, onUpdate = null) {
      return __awaiter(this, void 0, void 0, function* () {
        ...
        // setup visited mask
        const visited = new typedarrays_4.BooleanArray2D(
          result.width,
          result.height,
        );
        // setup facet map & array
        result.facetMap = new typedarrays_4.Uint32Array2D(
          result.width,
          result.height,
        );
        result.facets = [];
...
define("facetLabelPlacer", [
  ...
  class FacetLabelPlacer {
     ...
     */
    static doesNeighbourFallInsideInCurrentFacet(
      neighbourPath,
      f,
      onlyOuterRing,
    ) {
      ...
      if (fallsInside) {
        ...
        for (let i = 0; i < neighbourPath.length && fallsInside; i++) {
          const distance = polylabel_1.pointToPolygonDist(
            neighbourPath[i].x,
            neighbourPath[i].y,
            onlyOuterRing,
          );
  ...
  class GUIProcessManager {
    static process(settings, cancellationToken) {
      return __awaiter(this, void 0, void 0, function* () {
        ...
        // k-means clustering
        const kmeansImgData = yield GUIProcessManager.processKmeansClustering(
          imgData,
          tabsOutput,
          ctx,
          settings,
        ...
        if (settings.narrowPixelStripCleanupRuns === 0) {
          ...
          // facet reduction
          yield GUIProcessManager.processFacetReduction(
            facetResult,
            tabsOutput,
            settings,
            colormapResult,
          ...
          for (let run = 0; run < settings.narrowPixelStripCleanupRuns; run++) {
            ...
            yield colorreductionmanagement_2.ColorReducer.processNarrowPixelStripCleanup(
              colormapResult,
            );
            // facet building
            facetResult = yield GUIProcessManager.processFacetBuilding(
        ...
        yield GUIProcessManager.processFacetBorderTracing(
          tabsOutput,
          facetResult,
          cancellationToken,
        );
    ...
    static processKmeansClustering(
      imgData,
      tabsOutput,
      ctx,
      settings,
      ...
      return __awaiter(this, void 0, void 0, function* () {
        ...
        yield colorreductionmanagement_2.ColorReducer.applyKMeansClustering(
          imgData,
          kmeansImgData,
          ctx,
          settings,
    ...
    static processFacetReduction(
      ...
      return __awaiter(this, void 0, void 0, function* () {
        ...
        yield facetReducer_1.FacetReducer.reduceFacets(
          ...
          (progress) => {
            ...
            for (let j = 0; j < facetResult.height; j++) {
              for (let i = 0; i < facetResult.width; i++) {
                const facet =
                  facetResult.facets[facetResult.facetMap.get(i, j)];
    ...
    static processFacetBorderTracing(
      tabsOutput,
      facetResult,
      cancellationToken,
    ) {
    ...
    static processFacetBorderSegmentation(
      facetResult,
      tabsOutput,
      settings,
      cancellationToken,
      ...
      return __awaiter(this, void 0, void 0, function* () {
        ...
        yield facetBorderSegmenter_1.FacetBorderSegmenter.buildFacetBorderSegments(
          ...
          (progress) => {
            ...
            // update status & image
            $("#statusFacetBorderSegmentation").css(
              "width",
              Math.round(progress * 100) + "%",
            );
    ...
    static processFacetLabelPlacement(
      facetResult,
      cBorderSegment,
      tabsOutput,
      cancellationToken,
...
define("gui", [
  ...
  function process() {
    return __awaiter(this, void 0, void 0, function* () {
      try {
        ...
        processResult = yield guiprocessmanager_1.GUIProcessManager.process(
          settings,
          cancellationToken,
        );
        yield updateOutput();
  ...
  function downloadPalettePng() {
    ...
    for (let i = 0; i < colorsByIndex.length; i++) {
      ...
      const y =
        margin + Math.floor(i / nrOfItemsPerRow) * (cellHeight + margin);
